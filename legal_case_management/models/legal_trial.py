...
class LegalTrial(models.Model):
    ...
    is_invoiced = fields.Boolean(string='Is invoiced', help="Is trial invoiced",
                                 default=False)
    trial_number = fields.Char(string='Trail No', tracking=True)
    ...
    def remainder_trail_date(self):
        """trail date reminder"""
        today_date = datetime.today().date()
        trials = self.env['legal.trial'].search([('trial_date', '!=', False)])
        for record in trials:
            if record.trial_date:
                adjusted_date = record.trial_date - timedelta(days=1)
                adjusted_date_only = adjusted_date.date()
                if adjusted_date_only == today_date:
                    template = self.env.ref('legal_case_management.trail_reminder_date')
